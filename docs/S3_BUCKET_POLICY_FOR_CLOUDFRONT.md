...
## ⚠️ Vercel 환경 변수 (일반 업로드 이미지 401일 때 먼저 확인)

**일반(압축) 업로드** 이미지가 401로 깨지면, 프록시가 **CloudFront URL**이 아니라 **S3 직접 URL**을 쓰고 있을 수 있습니다. S3 버킷은 CloudFront(OAC)로만 열려 있어서 직접 URL은 401입니다.
...
3. 저장 후 **재배포** (또는 최신 배포가 해당 env를 쓰는지 확인).

설정이 없으면 `/api/photo/proxy`는 503을 반환하고, body에 `hint`로 위 안내가 나옵니다.

---

## ⚠️ Cloudinary: Fetched URL 허용 (401 나올 때 꼭 확인)

**일반 업로드**는 프록시 → Cloudinary **fetch** URL로 보냅니다. Cloudinary에서 **Fetched URL**이 제한되어 있으면, 서명 없는 fetch 요청에 **401 Unauthorized**를 반환합니다. (S3/CloudFront는 정상이어도 401이 납니다.)

1. **Cloudinary 대시보드** 로그인 → https://console.cloudinary.com (또는 dashboard.cloudinary.com)
2. **Settings** (또는 제품 환경 설정) → **Security** (보안) 탭
3. **Restricted media types** (제한된 미디어 유형)에서 **「Fetched URL」** 항목 찾기
4. **「Fetched URL」이 체크되어 있으면** → **체크 해제** (fetch 허용)
5. 저장 후 즉시 반영됨. (이미 캐시된 401이 있으면 1–2분 뒤 또는 강력 새로고침 후 다시 시도)

**더 안전하게 쓰려면 (Fetched URL 제한 유지):**  
「Fetched URL」을 다시 체크한 뒤, **Allowed fetch domains**에 CloudFront 도메인만 넣으면, 그 도메인에서 오는 URL만 fetch 가능합니다.

1. **Settings** → **Security** 탭에서 아래로 내려가 **「Allowed fetch domains」** (허용 fetch 도메인) 항목 찾기.  
   (제품/환경별로 있을 수 있음. 없으면 플랜에 따라 비노출일 수 있음.)
2. **Add domain** 또는 입력란에 **CloudFront 호스트만** 추가.  
   - 예: `d1bjjw498g4ixc.cloudfront.net`  
   - `https://` 없이, **호스트명만** 입력. (Vercel의 `AWS_CLOUDFRONT_DOMAIN` 값과 동일.)
3. 저장 후 **「Fetched URL」을 다시 체크**(제한 켜기).  
   - 이제 CloudFront 도메인에서 오는 URL만 fetch 가능하고, 다른 외부 URL은 401.
4. 앱에서 이미지 다시 로드해 보기.
