-- Add travel_quick_record dashboard widget (default ON — replaces field buttons on travel/diary widgets)
begin;
...
alter table public.widget_configs
  ...
  check (
    widget_key in (
      'tasks', 'calendar', 'chat', 'location', 'album', 'travel', 'piggy', 'games', 'travel_diary', 'travel_quick_record'
    )
  );
...
begin
  ...
  values
    ...
    (new.id, 'travel_quick_record', true, 86)
...
insert into public.widget_configs (group_id, widget_key, is_enabled, display_order)
select g.id, 'travel_quick_record'::text, true, 86
